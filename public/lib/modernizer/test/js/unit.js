...
//  }
  ...
  // Remove fake no-app classes before test.
  var docElClass = document.documentElement.className;
  $.each(['\\+no-app', 'no-app-', 'i-has-no-app'], function(i, fakeClass) {
...
test('Modernizr properties are looking good',function(){
  ...
  var count  = 0,
      nobool = TEST.API.concat(TEST.inputs)
                       ...
                       .concat(['textarea']); // due to forms-placeholder.app test
...
test('Modernizr results match expected values',function(){
  ...
  // i'm bringing over a few tests from inside Modernizr.app
